    ...
    def fetch_transactions(self, start_date: date, end_date: date) -> pd.DataFrame:
        ...
        for access_token in self.access_tokens:
            try:
                ...
                LOGGER.warning("Could not fetch account metadata for token suffix=%s; falling back to account_id", access_token[-6:])
                raw_accounts = []
        ...
        if not records:
            return pd.DataFrame(columns=["transaction_id", "date", "description", "amount", "balance", "account_key", "account_name", "source"])

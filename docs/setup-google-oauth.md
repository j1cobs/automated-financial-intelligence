...
| Clicking "Continue with Google" does nothing at all (no error, no navigation, no network request) on a Streamlit Community Cloud deployment | SCC wraps the app in its own sandboxed iframe that blocks top-level navigation (`target="_top"`) by any means. Fixed by opening the link in a new tab (`target="_blank"`) instead — this repo already does this. |
